...
PRINT *, '-----------------# REAL (single precision)'    ! You might notice, FORTRAN doesn't use indentation, thus it won't matter
    ! testing out REAL
    ...
    PRINT '(f0.30)', j          ! You can notice that the maximum digit is of 20 decimal point length (for some unknown reasons)
                                !This seems random, this maybe just is the digital conversion of the binary number that was stored
                                !in the system. Usually shown upto the precise value with 2 more insignificant digits

PRINT *, '------------------# REAL*8 (DOUBLE PRECISION)' 
    PRINT *, j                  ! By default, the output has 6-7 digit precision (2 extra digits are random) which is expected
    PRINT *, k                  ! As expected, the output has ~17 digit precision, as if we do log_10(2^52)~16.9 [52 mantissa bits]
