program HO
    ...
    REAL, PARAMETER :: h = 6.626*10.0**(-34), NA = 6.022*10.0**(23), kf = 1896.0, PI = 4*ATAN(1.0)
    REAL :: M1 = 0.012, M2 = 0.016, mu, nu0, alpha, xmax
    INTEGER :: n, i
    REAL :: x(dim), Psi_array(dim, 0:nmax), dx, Psi                    ! in Fortran, we can define starting index too
...
recursive real function factorial(n) result(o)
    INTEGER :: n
    if (n == 0 .or. n == 1) then
        o = 1
        else
...
recursive real function hermite(n,y) result(Herm)
    INTEGER :: n
    REAL :: y 
    if (n == 0) then
        Herm = 1
...
!! This is not a good practice in Fortran, since, the variables like `alpha` is taken from the main function, but the other functions are
!! not contained in the main function. Which means these getting implicitly typed, which is bad.
